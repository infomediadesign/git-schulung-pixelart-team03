...
int main() {
    ...
    while (!WindowShouldClose()) // Detect window close button or ESC key
    {
        ...
        if (IsKeyDown(KEY_SPACE))
        {
            ...
            float camO = camera.offset.x;
            float camH = camera.offset.y;
            float spritePosX = (scrW / 2) + ((camO * (-1))-(Sprite.width/2));
            float spritePosY = (scrH / 2) + ((camH * (-1))-Sprite.height/2);
        }

        float spritePosX = (960 / 2) + ((camera.offset.x * (-1))-(Sprite.width/2));
        float spritePosY = (540 / 2) + ((camera.offset.y * (-1))-Sprite.height/2);
        ...
        std::string offsetX = std::to_string(camera.offset.x);
        std::string offsetY = std::to_string(camera.offset.y);
        ...
        //draw Text with Camera offsets "offsetX" and "offsetY" into the relative Position of left upper corner of the camera
        DrawText(TextFormat("offsetX: %s", offsetX.c_str()), (camera.offset.x)*(-1)+10, (camera.offset.y)*(-1)+10, 20, BLACK);
        DrawText(TextFormat("offsetY: %s", offsetY.c_str()), (camera.offset.x)*(-1) +10, (camera.offset.y)*(-1)+30, 20, BLACK);
        DrawText(TextFormat("screenHeight: %s", scrHeight.c_str()), (camera.offset.x)*(-1)+10, (camera.offset.y)*(-1)+50, 20, BLACK);
        DrawText(TextFormat("screenWidth: %s", scrWidth.c_str()), (camera.offset.x)*(-1)+10, (camera.offset.y)*(-1)+70, 20, BLACK);
        DrawText(TextFormat("monitorHeight: %s", monitorHeight.c_str()), (camera.offset.x)*(-1)+10, (camera.offset.y)*(-1)+90, 20, BLACK);
        DrawText(TextFormat("monitorWidth: %s", monitorWidth.c_str()), (camera.offset.x)*(-1)+10, (camera.offset.y)*(-1)+110, 20, BLACK);
